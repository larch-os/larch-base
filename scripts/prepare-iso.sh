...
# submodules need bumping (or just run it every time -- it's idempotent,
# each step only redoes work that's actually stale).
set -euo pipefail
...
AUR_PACKAGES=(herdr-bin k3d-bin)
...
build_larch_calamares() {
    mkdir -p "$LOCAL_REPO" "$BUILD_DIR"

    local calamares_dir="$BUILD_DIR/larch-calamares"
...
fetch_wallpapers() {
    ...
    local count=0
    while curl -fsSL "$WALLPAPER_BASE_URL/larch-$count.png" -o "$WALLPAPER_DEST_DIR/larch-$count.png"; do
        count=$((count + 1))
    done
    rm -f "$WALLPAPER_DEST_DIR/larch-$count.png"  # curl -f still creates this on the 404 that ended the loop
